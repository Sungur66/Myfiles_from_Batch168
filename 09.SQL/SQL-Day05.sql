...
--musteri_isim alanında "Lary" varsa o degeri "No name" olarak güncelleyin.
...
-- maaslar ortalama maastan az ise maaslari ortalama maasa esitle
...
--Maaşlar ortalama maaştan az ise maaşları 1000 artırın

UPDATE workers
SET worker_salary = worker_salary * 1000
WHERE worker_salary < (SELECT AVG(worker_salary) FROM workers);

--"Brad Pitt"in maaşını en yüksek maaşın yüzde 20 sinden daha dusuk
-- olacak sekilde  yukseltiniz
UPDATE workers
SET worker_salary = (SELECT MAX(worker_salary)*0.80 FROM workers)
WHERE worker_name = 'Brad Pitt';

--Tom Hanks'in maaşını en düşük maaştan yüzde 30 daha fazla olacak sekilde düşürün
UPDATE workers
...
-- Tüm null degerleri "isim daha sonra eklenecek" olarak degistirin
UPDATE people
SET name = 'isim daha sonra eklenecek'
WHERE name is NULL;

--Tüm boş adres değerlerini "Adres daha sonra eklenecek" olarak değiştirin
UPDATE people
SET address = 'Adres daha sonra eklenecek'
WHERE address is NULL;

-- tablodan spesifik bir kayit nasil silinir

DELETE FROM people
WHERE name = 'Maryy Tien';

-- Ad degeri olmayan kayitlar nasil silinir?
DELETE FROM people
...
DELETE FROM people
WHERE name is NULL OR address ISNULL;
...
-- IS NOT NULL
-- isim degeri null olmayan tüm kayitlari silin
DELETE FROM people
...
--Interwiew Q: Maasi en düsük ve en yüksek olan kayitlar nasil secilir?
--1.yol
SELECT * FROM workers
...
select * from workers
WHERE worker_salary= (SELECT MAX(worker_salary) FROM workers WHERE worker_salary<(SELECT max(worker_salary) FROM workers));
...
SELECT * FROM workers
ORDER BY worker_salary desc -- asc artan sekilde, desc azalan sekilde, default naturel order
OFFSET 2  --kac kayidi es gececegi
FETCH NEXT 1 ROW ONLY -- kac deger döndürecegi, burda bir deger
